...
class FieldTest: XCTestCase {
    ...
    func testAddPlayer() throws {
        ...
        let human = Human(pos: Position(x: 1, y: 1), id: 0, logic: Logic())
        field.addPlayer(player: human)
        XCTAssertEqual(field.getPlayers(x: 1, y: 1).count, 1)
        let human2 = Human(pos: Position(x: 3, y: 0), id: 1, logic: Logic())
    ...
    func testBlockCommand() throws {
        let field = Field()
        let human = Human(pos: Position(x: 0, y: 1), id: 0, logic: Logic())
        let human2 = Human(pos: Position(x: 1, y: 0), id: 0, logic: Logic())
